...
%typing::default_type = (
  ...
  'procedure' => typing::make_type ('procedure',0,"procedure"),
  'generic'   => typing::make_type ('generic',0,"generic"),
  'final'     => typing::make_type ('final',0,"final"),
);
...
$typing::wild_type = {
   ...
   'procedure' => typing::make_type ('procedure'),
   'generic'   => typing::make_type ('generic'),
   'final'     => typing::make_type ('final'),
};
...
sub type_to_f90 {
  ...
  if ($type->{'base'} eq "character") {
    ...
    $mods = "$type->{'sub'}";
  } elsif ($type->{'base'} eq "class") {
    $mods = "$type->{'sub'}";
  } elsif ($type->{'base'} eq "procedure") {
    $mods = "$type->{'sub'}";
  } elsif ($type->{'base'} eq "generic") {
    $mods = "$type->{'sub'}";
  } elsif ($type->{'base'} eq "final") {
    $mods = "$type->{'sub'}";
  } else {
    $mods = "kind=" . expr_to_f90 ($type->{'sub'});
  }
