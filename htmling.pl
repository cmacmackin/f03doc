  ...
  if (@_) {
    ...
    foreach $struct (@_) {
      ...
      if ($type eq "var") {
        ...
        print OUT type2str ($struct, $href);
        print OUT " (" . join (", ", @{$struct->{'parms'}}) . ")"
          if exists $struct->{'parms'};
        print OUT "\n";
      } else {
        print OUT join (" ", attriblist ($struct), "");
        print OUT "$type $href\n";
...
sub do_html {
   if (@_) {
      ...
      foreach $struct (@_) {
         ...
         if ($type eq "typebound" && !$typebound) {
            print OUT $htmling::indentspace x ($htmling::indent - 1), "contains\n";
            $typebound = 1;
         }
         ...
         if ($type eq "var" ) {
         ...
         } elsif ($type eq "typebound") {
             print OUT proc2html ($struct) . "\n";
             ...
             print OUT $struct->{'bind'} if exists $struct->{'bind'};
             print OUT "\n";
         } elsif ($type eq "type") {
             print OUT type2str ($struct, $name);
           print OUT " (" . join (", ", @{$struct->{'parms'}}) . ")"
             if exists $struct->{'parms'};
           print OUT "\n"; 
         } else {
         ...
         if ($type eq "var" || $type eq "mprocedure" || $type eq "typebound") {
         } elsif ($type eq "type") {
           ...
           my $contains = do_html (@{$struct->{'ocontains'}});
           print OUT $htmling::indentspace x ($htmling::indent - 1), "contains\n" if !$contains;
           print OUT $htmling::indentspace x $htmling::indent, "final :: " . final2html(@{$struct->{'finalization'}}) . "\n" if (scalar @{$struct->{'finalization'}} > 0);
         ...
         if ($type ne "var" && $type ne "typebound" && $type ne "mprocedure") {
...
sub proc2html {
    ...
    my ($typestr) = typing::type_to_f90 ($proc->{'vartype'});
    my $href = txt2html ($proc->{'name'}) ;
    if (scalar @{$proc->{'modprocs'}} eq 1 && $proc->{'name'} eq $proc->{'modprocs'}[0]->{'name'}) {
       $href = "<A HREF=\"${htmling::htmlfile}#" . ${proc}->{'modprocs'}[0]->{'bind'}->{'type'} . "_" . lc ($proc->{'name'}) . "\">$proc->{'name'}</A>";
    } else {
       $href = $href . " => ";
       my $binding;
       my $first = 1;
       foreach $binding (@{${proc}->{'modprocs'}}) {
          $href = $href . ", " if !$first;
          $first = 0 if $first; 
          $href = $href . "<A HREF=\"${htmling::htmlfile}#" . $binding->{'bind'}->{'type'} . "_" . lc ($binding->{'name'}) . "\">$binding->{'name'}</A>";
       }
    }
    return $typestr . join (", ", "", attriblist ($proc)) . " :: $href";
}
...
sub final2html {
    ...
    foreach $f (@finals) {
       $href = $href . ", " if !$first;
       $first = 0 if $first; 
       $href = $href . "<A HREF=\"${htmling::htmlfile}#subroutine_$f\">$f</A>";  
    }
    return $href;
}
...
sub type2str {
    ...
    $href = txt2html ($type->{'name'}) unless $href;
    return "type" . join (", ", "", attriblist ($type)) . " :: $href";
}
