...
sub read_stmt {
   ...
   # END MODULE/SUBROUTINE/FUNCTION/PROGRAM/TYPE/INTERFACE, or general END
      ...
      if ($top->{'type'} eq "module") {
        ...
        foreach $name (@{$top->{'protectedlist'}}) {
          do_attrib ($name, "vis", 'protected', "visibility");
        }
        ...
        sub traverse {
          ...
          # Graduate TYPE-BOUND PROCEDURE (tbrocedure) to point to the
          # appropriate things (a function, subroutine or interface with that
          # name).
          } elsif ($node->{'type'} eq "typebound") {
            my $binding;
            foreach $binding (@{$node->{'modprocs'}}) {
              die "Couldn't find module procedure $binding->{'name'} (nothing with that name in module $top->{'name'})"
                unless exists $top->{'contains'}->{lc $binding->{'name'}};
              my ($possibles) =
                $top->{'contains'}->{lc $binding->{'name'}};
              die "Couldn't find module procedure $binding->{'name'} in module $top->{'name'} (wrong type)"
                if !exists $possibles->{'subroutine'}
                && !exists $possibles->{'function'} 
                && !exists $possibles->{'interface'};
              die "Found more than one of subroutine, function, and interface to match module procedure $binding->{'name'} in module $top->{'name'}"
                if ( (exists $possibles->{'subroutine'}
                && exists $possibles->{'function'}) || 
                (exists $possibles->{'subroutine'}
                && exists $possibles->{'interface'}) ||
                (exists $possibles->{'interface'}
                && exists $possibles->{'function'}) );
              
              if (exists $possibles->{'subroutine'}) {
                $binding->{'bind'} = $possibles->{'subroutine'};
              } elsif (exists $possibles->{'function'}) {
                $binding->{'bind'} = $possibles->{'function'};
              } else {
                $binding->{'bind'} = $possibles->{'interface'};
              }
            }
          }
   ...
   # SUBROUTINE/FUNCTION
   elsif ($line =~ /^(?:(.+?)\s+)?(subroutine|function)\s+(\w+)\s*(\([^()]*\))?(?:\s*result\s*\(\s*(\w+)\s*\))?(\s*bind\s*\(\s*c.*\))?$/i) {
      my ($type, $name, $parmstr, $rtype, $result, $bind) =
         (lc $2, $3,    $4,       $1,     $5,      $6);
      ...
      $struct->{'bind'} = ' ' . typing::expr_to_f90 (expr_parse::parse_expr ($bind)) if defined $bind;
      return ($type, $struct);
   }

   # TYPE definition (must go before variable declarations)
   elsif ($line =~ /^type(?:\s+|\s*(,.*)?::\s*)(\w+)\s*(\([^()]*\))?$/i) {
     my $attribstr = $1 if defined $1;
     my $name = $2 if defined $2;
     my $parmstr = $3 if defined $3;
     my @tmp = ();
     ...
     if (defined $attribstr) {
       my @attribs = map (( utils::trim ($_) ), utils::balsplit (",", $attribstr));
       ...
       foreach $attrib (@attribs) {
         if ($attrib =~ /^(public|private)$/i) {
           $struct->{'vis'} = lc $attrib;
            ...
            $attrib = '' . typing::expr_to_f90 (expr_parse::parse_expr ($attrib)) if $attrib =~ /'.*'/;
     ...
     if (defined $parmstr) {
       ...
       if ($parmstr) {
          ...
          my ($parm);
          foreach $parm (@parms) {
             die "Parameter `$parm' is not just a word"
               unless $parm =~ /^\w+$/;
          }
       } else {
         @parms = ();
   ...
   # Abstract INTERFACE
   elsif ($line =~ /^abstract\s+interface\s*$/i){
       return new_nest ({
           'type' => 'interface',
           'name' => "",
           'tempattribs' => ['abstract'],
           hashed_comments ()
       });
   } 
   ...
   elsif ($line =~ /^(public|private|protected|sequence)(?=\s+[^=(]|::|$)(\s*::\s*)?/i) {
     my ($what, $rest) = (lc $1, $');
   ...
   elsif ($line =~ /^(generic|procedure)\s*(\(|\s\w|[:,*])/i && 
    defined $stmts::topnest && $stmts::topnest->{'incontains'} && 
    $stmts::topnest->{'type'} eq "type") {
      my ($vartype, $rest) = parse_part_as_type ($line);
      my (@attribs, $binding, @modprocs);
      if ($rest =~ /^(.*)\:\:\s*(.*)\s*=>\s*((\w(\s*,\s*)?)*)/) {
         my ($a, $b, $c) = ($1, $2, $3); 
         @attribs = map (( utils::trim ($_) ), utils::balsplit (",", $a));
         $binding = utils::trim ($b);
         @modprocs = map (( utils::trim ($_) ), utils::balsplit (",", $c));
      } elsif ($rest =~ /^(.*)\:\:\s*(\w+)/) {
         my ($a, $b) = ($1, $2);
         @attribs = map (( utils::trim ($_) ), utils::balsplit (",", $a));
         $binding = utils::trim ($b);
      ...
      push @modprocs, $binding if !@modprocs;

      my ($proc, @proclist);
      foreach $proc (@modprocs) {
         my $newproc = {
            'type'   => "tbprocedure",
            'name'   => $proc
         };
         push @proclist, $newproc;
      }
      $struct->{'modprocs'} = \@proclist;
   ...
   # FINAL subroutines
    ...
    $stmts::topnest->{'type'} eq "type") {
        $line =~ /^(final)\s*::/i;
        my @routines = map (( utils::trim ($_) ), utils::balsplit (",", $'));
        my ($r);
        foreach $r (@routines) {
           push @{$stmts::topnest->{'finalization'}}, $r;
        }
        print ''; #Don't know why I need this in here, but things don't work without it
    }

   # Variable declarations (and finalization bindings)
   elsif ($line =~ /^(integer|real|double\s*precision|character|complex|logical|type|class|procedure)\s*(\(|\s\w|[:,*])/i) {
      my ($vartype, $rest) = parse_part_as_type ($line);
      my (@attribs, @right);
      if ($rest =~ /^(.*)\:\:(.*)/) {
         my ($a, $b) = ($1, $2);
      ...
      foreach $r (@right) {
          ...
          my ($attrib, @tempattribs);
          foreach $attrib (@attribs_copy) {
              if ($attrib =~ /^(public|private|protected)$/i) {
                  $attrib = lc $attrib;
                  $struct->{'vis'} = $attrib;
              } elsif ($attrib =~ /^optional$/i) {
                  $attrib = lc $attrib;
...
#####
# Makes note of a new type-bound procedure structures.
#####
sub new_tbproc {
   my ($struct) = @_;
   my $type = $struct->{'type'};
...
sub new_nest {
   ...
   # Program unit
   if ($type eq "subroutine" || $type eq "function" || $type eq "module" || $type eq "program" || $type eq "type") {
     $struct->{'incontains'} = 0;
     $struct->{'uses'} = [ ];
...
sub parse_part_as_type {
  ...
  $str =~ /^integer|real|double\s*precision|character|complex|logical|type|class|procedure|generic|final/i
...
sub parse_type {
  ...
  $str = utils::trim ($str);
  $str =~ /^(integer|real|double\s*precision|complex|character|logical|type|class|procedure|generic|final)
    \s* (?: \( (.*) \) | \* \s* (\d+ | \(.*\)) )?$/ix
  ...
  if (defined $2 || defined $3) {
    ...
    if ($base eq 'type') {
      ...
      die "class$args invalid--use class($args)" if $star;
      die "class(w) for non-word w" unless $args =~ /^(\w+|\*)$/;
      return typing::make_type ($base, $args);
    } elsif ($base eq 'procedure') {
      die "procedure$args invalid--use procedure($args)" if $star;
      die "procedure(w) for non-word w" unless $args =~ /^(\w+)$/;
      return typing::make_type ($base, $args);
    ...
    die "type or class without (type-name) after it" if ($base eq 'type' || $base eq 'class');
